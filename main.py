...
from core.trainer import run_quiz, load_progress, prepare_pairs, Traduction, make_progress_path
...
def get_progress_info(file_path: Path):
    ...
    try:
        ...
        return sum([1 for traduction in data if not traduction["correct"]])
    except:
        return None
...
def main():
    ...
    if use_saved:
        traductions = load_progress(selected_file)
    else:
        ...
        traductions = [Traduction(p, a) for p, a in all_pairs]
    
    run_quiz(traductions, selected_file)
